...
/// Kernel interrupts (also throwing GDT stuff here for now)use x86_64::VirtAddr;
    ...
    _stack_frame: InterruptStackFrame)
{
    ...
    if let Ok(Some(key_event)) = keyboard.add_byte(scancode) {
        if let Some(key) = keyboard.process_keyevent(key_event) {
            match key {
                DecodedKey::Unicode(character) => sprint!("{}", character),
                DecodedKey::RawKey(key) => sprint!("{:?}", key),
            }
        }
    }
